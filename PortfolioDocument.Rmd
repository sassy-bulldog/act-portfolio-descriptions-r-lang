...
if (local_env$capital_call_file_missing == FALSE) {
  ...
  local_env$historic_periodic_capital_calls <- local_env$capital_model %>%
    ...
    select(Program, LineOfBusiness, all_of(local_env$current_year_columns)) %>%
    mutate(across(all_of(local_env$current_year_columns), ~ as.numeric(gsub("[^0-9.-]", "", .))))
  ...
  local_env$historic_cumulative_capital_calls <- local_env$capital_model %>%
    ...
    filter(!Program %in% c("LOD", "Risk Attaching", "0")) %>%
    select(Program, LineOfBusiness, all_of(local_env$previous_year_column)) %>%
    mutate(across(all_of(local_env$previous_year_column), ~ as.numeric(gsub("[^0-9.-]", "", .))))
  ...
  local_env$historic_capital_calls <- local_env$historic_periodic_capital_calls %>%
    ...
    select(Program, LineOfBusiness, all_of(local_env$previous_year_column), all_of(local_env$current_year_columns)) %>%
    mutate(
      Total = rowSums(
        across(
...
if (local_env$capital_call_file_missing == FALSE) {
  ...
  local_env$projected_cumulative_capital_calls <- local_env$capital_model %>%
    filter(Measurement == "Collateral Called [Cumulative]") %>%
    filter(!Program %in% c("LOD", "Risk Attaching", "0")) %>%
    select(Program, LineOfBusiness, all_of(local_env$next_year_end_column)) %>%
    mutate(across(all_of(local_env$next_year_end_column), ~ as.numeric(gsub("[^0-9.-]", "", .))))
  ...
  local_env$projected_capital_calls <- flextable::set_header_labels(
  ...
  local_env$projected_capital_calls <- flextable::bold(local_env$projected_capital_calls, bold = TRUE, part = "header")
  local_env$projected_capital_calls <- flextable::fontsize(local_env$projected_capital_calls, size = 8, part = "all")
  local_env$projected_capital_calls <- flextable::font(local_env$projected_capital_calls, fontname = "Arial", part = "all")
  local_env$projected_capital_calls <- flextable::set_table_properties(local_env$projected_capital_calls, width = 1, layout = "autofit")
  local_env$projected_capital_calls <- flextable::bg(
    local_env$projected_capital_calls,
    i = which(
